...
define( 'OLR_CHECKOUT_TEST_VERSION', '1.4.4' );
...
/** Display the native shipping total independently of the relocated rate picker. */
function olr_checkout_test_shipping_summary() {
	if ( ! function_exists( 'WC' ) || ! WC()->session || ! WC()->cart || ! WC()->session->get( 'olr_checkout_test' ) || ! WC()->cart->needs_shipping() ) {
		return;
	}
	$shipping = WC()->cart->has_calculated_shipping()
		? WC()->cart->get_cart_shipping_total()
		: __( 'Calculated after address', 'off-label-checkout-test' );
	echo '<tr class="olr-checkout-shipping-summary"><th scope="row">' . esc_html__( 'Shipping', 'off-label-checkout-test' ) . '</th><td data-title="' . esc_attr__( 'Shipping', 'off-label-checkout-test' ) . '">' . wp_kses_post( $shipping ) . '</td></tr>';
}
add_action( 'woocommerce_review_order_before_shipping', 'olr_checkout_test_shipping_summary' );
