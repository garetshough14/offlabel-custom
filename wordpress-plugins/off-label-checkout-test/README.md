...
Current build: **1.3.10**. This release restores the established checkout presentation, keeps the compact mobile column safeguard with a consistent 20px phone gutter, excludes unnecessary advertising pixels from the noindex test route, waits for WordPress.com's real jQuery runtime instead of its temporary head shim, and releases a failed initial WooCommerce totals overlay instead of leaving order items permanently faded and blocked. It retains the global GitPress header/footer and all existing checkout behavior.
