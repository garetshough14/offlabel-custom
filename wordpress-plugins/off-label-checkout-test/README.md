...
Current build: **1.3.8**. This release keeps the progress controls transparent, restores their intended contrast, and pins the checkout form to a single mobile column so late WooCommerce or theme grid rules cannot collapse the contact fields at 320px. It retains the final wrapping guards, global GitPress header/footer, and all existing checkout behavior.
