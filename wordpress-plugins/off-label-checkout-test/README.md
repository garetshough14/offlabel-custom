...
Current build: **1.4.2**. This release promotes the approved Checkout 1.3.11 layout without redesigning it. It preserves the checkout-owned trust strip, logo header, wide two-column desktop canvas, compact order rows, mobile padding, responsive footer, and failed-totals-overlay recovery. The recovery now watches every checkout step and every order-review refresh, so a late or failed WooCommerce totals request cannot leave the products under a permanent grey overlay. It also removes the non-charging test gateway and all staging-only order behavior.
