...
Current build: **1.1.8**. Test routes reuse the production plugin asset handles so WordPress emits each stylesheet and script once, and include scoped staging guards for Ultimate Member menu widths, checkout progress-button contrast, and compact checkout grid placement.
