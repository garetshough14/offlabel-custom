...
class DGS_Page_Shortcode_Manager {
	...
	public static function init() {
		...
		// Rendering here used to exit before membership checks on template_redirect.
		add_filter( 'template_include', array( __CLASS__, 'select_full_page_template' ), 99 );
		add_action( 'wp', array( __CLASS__, 'prevent_member_page_cache' ), 0 );
	}
	...
	/**
	 * Select the canvas only after WordPress has completed template_redirect.
	 * Ultimate Member keeps ownership of login, role checks and public exceptions.
	 *
	 * @param string $template Original theme/plugin template.
	 * @return string
	 */
	public static function select_full_page_template( $template ) {
		return self::can_render_full_page() ? DGS_PLUGIN_DIR . 'includes/full-page-template.php' : $template;
	}

	/** Do not substitute a page canvas for service requests or Woo endpoints. */
	private static function can_render_full_page() {
		if ( is_admin() || wp_doing_ajax() || wp_doing_cron() || ! is_singular() || is_feed() || is_404()
		...
		$post = get_queried_object();
		return $post instanceof WP_Post && self::is_full_page_enabled( $post->ID )
			&& '' !== trim( (string) get_post_meta( $post->ID, self::META_KEY_SHORTCODE, true ) );
	}

	/** Keep member-dependent canvas responses out of cooperating page caches. */
	public static function prevent_member_page_cache() {
		if ( ! self::can_render_full_page() || ! function_exists( 'UM' ) ) {
			return;
		}
		$um = UM();
		if ( ! is_object( $um ) || ! is_callable( array( $um, 'options' ) ) ) {
			return;
		}
		$options = $um->options();
		if ( ! is_object( $options ) || ! is_callable( array( $options, 'get' ) ) || 2 !== (int) $options->get( 'accessible' ) ) {
			return;
		}
	...
	/**
	 * Render a repo-backed page from the selected template, never a redirect hook.
	 *
	 * @return void
	 */
	public static function maybe_render_full_page_canvas() {
		if ( ! self::can_render_full_page() ) {
			return;
		}
