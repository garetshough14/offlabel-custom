...
<main class="olr-coa-detail" data-olr-coa-viewer data-pdf-url="<?php echo esc_url( $pdf_url ); ?>">
	<section class="olr-coa-detail__hero">
		<div class="olr-coa-detail__identity">
			...
			<p>Independent testing verified for purity, identity and quality. View the full report below.</p>
		...
		<div class="olr-coa-detail__seal" aria-label="Verified <?php echo esc_attr( $purity_display ); ?> purity, third-party tested"><span>Verified</span><strong><?php echo esc_html( $purity_display ); ?></strong><b>Purity</b><em>Third-party tested</em></div>
	...
	<section class="olr-coa-detail__information">
		<article><h2>What you’re looking at</h2><i aria-hidden="true"></i><p>This Certificate of Analysis (COA) shows the results of third-party testing performed on this research product.</p><ul><li><strong>Test date:</strong> When the analysis was completed.</li><li><strong>Testing lab:</strong> The independent laboratory that performed the testing<?php echo $lab ? ': ' . esc_html( $lab ) : '.'; ?></li><li><strong>Method:</strong> <?php echo esc_html( $method ? $method : 'The analytical method used to test this product.' ); ?></li><li><strong>Purity / result:</strong> The measured purity result. Specification represents the minimum acceptable standard.</li></ul></article>
		<article><h2>Previous testing</h2><i aria-hidden="true"></i><?php if ( $reports ) : ?><div class="olr-coa-detail__history"><?php foreach ( $reports as $report ) : $history_pdf_id = absint( get_post_meta( $report->ID, '_olr_pdf_id', true ) ); ?><a href="<?php echo esc_url( wp_get_attachment_url( $history_pdf_id ) ); ?>" target="_blank" rel="noopener"><span><strong><?php echo esc_html( $this->display_date( get_post_meta( $report->ID, '_olr_test_date', true ) ) ); ?></strong><small>Tested by <?php echo esc_html( get_post_meta( $report->ID, '_olr_lab', true ) ?: 'independent laboratory' ); ?></small></span><b>View report&nbsp; →</b></a><?php endforeach; ?></div><?php else : ?><p>No previous published testing is available for this product.</p><?php endif; ?><a class="olr-coa-detail__history-all" href="<?php echo esc_url( home_url( '/coas/' ) ); ?>">View all testing history&nbsp; →</a></article>
	</section>
