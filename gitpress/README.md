...
The page body contains `[olr_research_catalog]`. The bridge renders the catalog from published WooCommerce products, including live images, names, prices, category counts, stock status, and stable ordering. All published, catalog-visible products appear on one page, including sold-out products with a badge; old `olr_page` links do not hide products. Category and sort controls are retained. Catalog cards point to `/catalog/{product-slug}/`. The retired editorial feature banner is not emitted; the shared stylesheet also suppresses that legacy class in case an older cached bridge briefly returns it. Replace the active WordPress copy of `gitpress/woocommerce-bridge.php` whenever this catalog shortcode changes.
